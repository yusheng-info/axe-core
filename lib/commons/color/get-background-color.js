...
color.filteredRectStack = function filteredRectStack(elm) {
	...
	// find the first parent with a clientWidth to see if the node
	// is smaller than the current elm (e.g. if parent is using
	// text ellipsis and elm is really long text)
	let node = elm;
	while (!node.clientWidth) {
		node = node.parentElement;
	}

	let rects = axe.utils.getNodeFromTree(elm).clientRects;
	let boundingRect = elm.getBoundingClientRect();
	if (node.clientWidth < boundingRect.width) {
		rects = axe.utils.getNodeFromTree(node).clientRects;
	}

	let rectStack = [];
	for (let i = 0; i < rects.length; i++) {
		let rect = rects[i];
		let vNodes = axe._cache.get('quadtree').get(rect);
		let result = vNodes
			.filter(vNode => {
				for (let x = 0; x < vNode.clientRects.length; x++) {
					let vNodeRect = vNode.clientRects[x];
					if (
						rect.x + rect.width / 2 < vNodeRect.x + vNodeRect.width &&
						rect.x + rect.width / 2 > vNodeRect.x &&
						rect.y + rect.height / 2 < vNodeRect.y + vNodeRect.height &&
						rect.y + rect.height / 2 > vNodeRect.y
					) {
						return true;
					}
				}
			...
			.sort(dom.visuallySort)
			.map(vNode => vNode.actualNode);

		// console.log('result:', result);
		rectStack.push(result);
	}
