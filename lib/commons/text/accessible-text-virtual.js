...
const { slottedSegment, slottedValue, slottedAttrs } = axe.constants;
...
function findLabel(virtualNode) {
	...
	// On DOM fragemtns it is impossible to tell
	// if there are labels elsewhere
	if (dom.isDetatched(virtualNode)) {
		return slottedSegment;
	}
...
function formValueText({ actualNode }, inLabelledByContext) {
	...
	if (nodeName === 'INPUT') {
		...
		if (actualNode.getAttribute('type') === slottedValue || actualNode.hasAttribute(slottedAttrs)) {
			return slottedSegment;
		}
		return '';
	}
...
function checkDescendant({ actualNode }, nodeName) {
	...
	if (candidate) {
		const virtualcandidate = axe.utils.getNodeFromTree(axe._tree[0], candidate);
		return text.accessibleArrayVirtual(virtualcandidate);
	}
...
function nonEmptyText(t) {
	return t === slottedSegment || !!text.sanitize(t);
}
...
text.accessibleTextVirtual = function accessibleTextVirtual(element, inLabelledByContext) {
	const textFragements = text.accessibleArrayVirtual(element, inLabelledByContext);
	return text.sanitize(textFragements.join(' '));
};
 ...
 * @return {string}
 */
text.accessibleArrayVirtual = function accessibleArrayVirtual(element, inLabelledByContext) {
	...
	function getInnerText (element, inLabelledByContext, inControlContext) {
		let innerText = element.children.reduce((returnText, child) => {
			const { actualNode } = child;
			if (actualNode.nodeType === 3) {
				returnText.push(actualNode.nodeValue);
			} else if (actualNode.nodeType === 1) {
				if (returnText.length > 0 && !phrasingElements.includes(actualNode.nodeName.toUpperCase())) {
					returnText.push(' ');
				}
				const childText = accessibleNameComputation(child, inLabelledByContext,
					inControlContext);
				returnText = returnText.concat(childText); 
			}
			return returnText;
		}, []);

		if (innerText.length === 0) {
			const isSlotted = Array.from(element.actualNode.children)
				.some(({ nodeName }) => nodeName.toUpperCase() === 'SLOT')
			if (isSlotted) {
				innerText = [slottedSegment]
			}
		}
		return innerText
	}
	...
	function getLayoutTableText (element) {
		...
		if (!axe.commons.table.isDataTable(element.actualNode) && axe.commons.table.getAllCells(element.actualNode).length === 1) {
			return getInnerText(element, false, false).map(txt => txt.trim());
		}
		return [];
	}
	...
	function checkNative (element, inLabelledByContext, inControlContext) {
		...
		if (shouldCheckSubtree(element)) {
			...
			if (returnText.some(nonEmptyText)) {
				return returnText;
			}
		}
		if (nodeName === 'FIGURE') {
		...
		if (nodeName === 'TABLE') {
			...
			const attrText = (actualNode.getAttribute('title') || actualNode.getAttribute('summary')) || '';
			if (nonEmptyText(attrText)) {
				return [attrText];
			}

			returnText = getLayoutTableText(element);
			if (returnText.some(nonEmptyText)) {
				return returnText;
			}
		}
	...
	function checkARIA (element, inLabelledByContext, inControlContext) {
		...
		if (!inLabelledByContext && actualNode.hasAttribute('aria-labelledby')) {
			...
			returnText = dom.idrefs(actualNode, 'aria-labelledby').reduce((arr, label) => {
				if (label !== null) {// handle unfound elements by dom.idref
					...
					const name = accessibleNameComputation(vLabel, true, actualNode !== label);
		...
		if (returnText.length === 0 && !(inControlContext && isEmbeddedControl(element)) &&
	...
	accessibleNameComputation = function (element, inLabelledByContext, inControlContext) {
		...
		} else if(!inLabelledByContext && dom.isVisible(element.actualNode, true) === false) {
			return [];
		}
	...
	return accessibleNameComputation(element, inLabelledByContext);
};
