...
axe.utils.querySelectorAllFilter = function(domTree, selector, filter) {
	...
	let matchedNodes = [];
	let selectorMap = domTree[0].selectorMap;

	// TODO: need to start at the provided DOM tree (how do we cache things
	// at each level so it remains fast?)

	// can cache the result of any parent lookups on the parent element so
	// we can short circuit a full parent tree traversal just like we did
	// for isHidden
	...
	expressions.forEach(expression => {
		...
		let tagName = exp.tag.toUpperCase();
		let id = exp.id;
		let nodes;

		// check tags
		if (tagName !== '*' && selectorMap[tagName]) {
		...
		if (tagName === '*' && !exp.attributes && !exp.id && !exp.classes) {
			nodes = selectorMap['*'];
		} else if (!nodes) {
			return;
		} else {
		...
		matchedNodes = matchedNodes.concat(nodes);
	});

	// if (window.performance && window.performance.mark) {
	// 	window.performance.mark('qsa-nodes-sort-start-' + selector);
	// }
	...
	// turns out this filter is super slow for 8K nodes
	// test for now to see if all rules pass
	// 	matchedNodes = matchedNodes.filter(function(value, index) {
	//   return matchedNodes.indexOf(value) === index;
	// });

	// unique list
	// how slow is this?
	matchedNodes = Array.from(new Set(matchedNodes));

	// if (window.performance && window.performance.mark) {
	// 	window.performance.mark('qsa-nodes-sort-end-' + selector);
	// 	window.performance.mark('qsa-end-' + selector);
	// 	window.performance.measure(
	// 		'qsa-nodes-sort-' + selector,
	// 		'qsa-nodes-sort-start-' + selector,
	// 		'qsa-nodes-sort-end-' + selector
	// 	);
	// 	window.performance.measure(
	// 		'qsa-' + selector,
	// 		'qsa-start-' + selector,
